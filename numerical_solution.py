...
class System():
    ...
    def calculate_H_elem(self, si: float, sj: float) -> float:
        ...
        """
        k = 0.5 * (si + sj)
        return (1.5 * sj * gaussian_integral(k , 2) + 
            0.5 * ( 1 - (sj) **2) * gaussian_integral(k,4) + 
            self.v0 * gaussian_integral( 0.5 * ( si + sj + 1/self.r0**2 ),2))
    ...
    def gen_random_state(self, size=None) -> float:
        ...
        return pow(np.random.uniform(self.r0 / 100 , 100 * self.r0,size),-2)
    ...
    def find_new_state(self, tries=2048):
        ...
        energy = lowest_eigenvalue(best_S,best_H)
        ...
        self.energies = np.append(self.energies,E_new)
    ...
    def construct_basis(self, nstates: int):#, max_failures=16):
        ...
        while np.size(self.states) < nstates:
            best_state= self.find_new_state()
            self.states = np.append(self.states, best_state)
...
if __name__ == "__main__":
    ...
    print("Building fit")
    print("============")
    system = System(r0 = 0.05 / np.sqrt(2), v0=-536.8013763987018)
    system.states = np.array(system.gen_random_state(2))
    ...
    #print("Random states test:" , system.gen_random_state(30))
    
    print('this is new basis\n', system.construct_basis(10))
    print('grounds state energies:\n', system.energies)
    print(len(system.energies))
    #for energy in system.best_energy:
      #  print(energy)
